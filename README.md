...
> **⚠️ Proyecto en construcción** — Versión de desarrollo v0.9.57. Este repositorio contiene el código fuente en evolución activa; las funcionalidades y la documentación pueden cambiar. Úsalo bajo tu propio criterio.
...
## Estado actual (v0.9.57)
- Base de datos `sistema_facturacion` con tablas `usuarios`, `impuestos`, `categorias`, `productos`, `clientes`, `proveedores`, `facturas`, `detalles_factura`, `detalle_impuestos`, `movimientos_inventario`, `configuraciones`, `resoluciones_dian` y `envios_dian` (Fase 6).
...
- **Ticket POS sin desbordes (v0.9.57)**: el CUFE/CUDE de la DIAN (96 caracteres) y cualquier otra línea larga (nombre de cliente/vendedor, teléfono, enlace de validación, totales) se parten y centran al ancho de la tira (48/80mm, 32/58mm) en vez de cortarse o desbordar la impresión.
- **Cliente real y modo estricto DIAN + alertas visibles (v0.9.58)**: el documento electrónico ahora usa los datos reales del cliente del catálogo — tipo de documento mapeado a código DIAN (`CC→13`, `NIT→31`, `CE→22`, `Pasaporte→41`), teléfono, correo y dirección (antes todo salía como CC); se **rechaza** el tipo "Otro" y un cliente registrado **sin documento** (pide ingresarlo), y con `dian_adquirente_consumidor=0` la Caja **exige un cliente real** para facturar (se eliminó el flag duplicado `dian_factura_obligatoria`, migración `backend/sql/18_eliminar_flag_obligatoria.sql`). En el frontend los avisos de error/éxito son **alertas flotantes con auto-cierre (5 s)** que ya no quedan ocultas detrás de los modales (`components/AlertaAuto.jsx`).
- Reportes de ventas, inventario y movimientos de inventario.
- **Frontend funcional**: Login, Dashboard con KPIs y gráficos, Caja (POS), Facturas, Productos, Catálogo, Clientes, Proveedores, Reportes y Usuarios. **Navegación superior (v0.9.55–56)**: navbar sticky de **dos filas** — fila 1 con marca y acciones, fila 2 con los menús por grupos (Dashboard, Operación, Inventario, Contactos, Reportes, Administración) que respetan los privilegios por rol; los menús desplegables se abren completos (sin recortes, el último grupo pegado al borde derecho) y la fuente es compacta para que nada se apriete. En móvil se conserva el menú deslizable con la misma navegación.
...
- **Caja UX (v0.9.59)**: en móvil (<992 px) ya no se muestra la tabla de escritorio aplastada (encabezados "Dto"/"Total" cortados); la venta se opera desde el carrito deslizante, que ahora incluye el **selector de impuestos por línea**. Editar la **cantidad** ya no elimina la línea: la casilla se **selecciona al enfocar** (teclear reemplaza sin borrar) y vaciarla restaura el valor previo (un "0" explícito sí elimina). Mismo `select()` al enfocar en descuentos. **Productos** suma una **fila de escaneo por código de barras** en el listado (pistola + Enter o cámara, visible con el flag activo).
- **Caja móvil con la venta a la vista (v0.9.60)**: la pantalla principal de la caja en móvil ya no queda vacía: muestra la **venta en curso** como lista táctil (nombre, precio unitario, cantidad y subtotal de cada ítem) y tocarla abre el carrito deslizante para ajustarla y cobrar; sin ítems muestra un aviso guía. El botón flotante con totales se conserva.
- **Facturación electrónica DIAN (Fase 6, v0.9.54)**: base de datos lista (`resoluciones_dian`, `envios_dian` con cola offline, columnas `cufe`/`estado_dian`/`xml_dian` en `facturas`, claves de configuración DIAN) + generador de XML **UBL 2.1** y cálculo del **CUFE (SHA-384**, anexo técnico vigente) en `backend/src/services/dian/`. Arquitectura por **adaptadores** (`factory.js`): en modo test (`simulacion.adapter.js`) la factura se "aprueba" localmente sin conexión a la DIAN, y la venta nunca se pierde si la DIAN falla (queda local y encolada para reintento). El adaptador **Factus** (`factus.adapter.js`, proveedor real) se activa con `dian_proveedor=factus`: publica en `POST /v2/bills/validate` y **guarda el CUFE oficial** de la DIAN; sus credenciales van en `.env` (`FACTUS_CLIENT_ID`/`FACTUS_CLIENT_SECRET`, migración `backend/sql/17_factus.sql`) y si no están configuradas la venta cae con seguridad al modo offline. La representación gráfica incluye **código QR + CUFE en el PDF** y **CUFE en el ticket POS** (librería `qrcode`). La pantalla **Facturas** muestra el **estado DIAN** (coloreado) con **CUFE** y botón **reintentar**; la **Caja** avisa al emitir si el documento quedó pendiente. La cola de reintentos se procesa automáticamente cada 5 min y con `POST /api/v1/dian/procesar-cola` (más `GET /api/v1/dian/pendientes`). La pantalla **Configuración** tiene un **bloque DIAN** (regímenes, proveedor y ambiente Factus como desplegables) y las **notas correctivas** con prefijos propios (NC/ND) por tipo.
- **Notas correctivas crédito/débito (v0.9.53)**: documentos electrónicos que corrigen una factura original ante la DIAN. Se crean desde la pantalla **Notas correctivas** (solo admin) eligiendo la factura original y el tipo; la **nota crédito** compensa la factura (por defecto el total = anulación electrónica) y la **nota débito** la incrementa con un monto de ajuste. Generan **XML UBL 2.1 `CreditNote`/`DebitNote`** con `BillingReference` a la factura y **CUDE** (SHA-384), se imprimen en **PDF** (QR + CUDE) y **ticket POS**, y tienen su cola de reintentos (`envios_dian_notas`). Migración `backend/sql/16_notas_correctivas.sql` (incluida en el maestro).
- Módulos pendientes: notas correctivas emitidas contra el proveedor real (Factus) y validación del CUFE real contra la DIAN con credenciales sandbox.
